...
const cameraMarker = computed(() => ({
  x: width / 2,
  y: Math.min(height - 80, Math.max(260, mapY2D(props.cameraPosition.x))),
}))
    ...
    <div class="relative min-h-0 flex-1 overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
      <svg :viewBox="`0 0 ${width} ${height}`" class="h-full w-full" preserveAspectRatio="xMidYMin meet">
        ...
        <g v-for="cabinet in cabinets" :key="cabinet.id">
          ...
          <text :x="mapX2D(cabinet.position[2])" :y="mapY2D(cabinet.position[0]) - panelHeight / 2 + 126" font-size="98" font-weight="800" :fill="selected.has(cabinet.upperId) ? '#0f172a' : '#1e293b'" text-anchor="middle" alignment-baseline="middle">{{ info(cabinet.upperId)?.unitId ?? '' }}</text>
          <text :x="mapX2D(cabinet.position[2])" :y="mapY2D(cabinet.position[0]) - panelHeight / 2 + 200" font-size="68" font-weight="700" :fill="selected.has(cabinet.upperId) ? '#b91c1c' : '#64748b'" text-anchor="middle" alignment-baseline="middle">{{ (info(cabinet.upperId)?.name ?? '').slice(0, 25) }}</text>
          <g v-if="cabinet.upperId !== cabinet.lowerId">
            <text :x="mapX2D(cabinet.position[2])" :y="mapY2D(cabinet.position[0]) - panelHeight / 2 + 338" font-size="98" font-weight="800" :fill="selected.has(cabinet.lowerId) ? '#0f172a' : '#1e293b'" text-anchor="middle" alignment-baseline="middle">{{ info(cabinet.lowerId)?.unitId ?? '' }}</text>
            <text :x="mapX2D(cabinet.position[2])" :y="mapY2D(cabinet.position[0]) - panelHeight / 2 + 412" font-size="68" font-weight="700" :fill="selected.has(cabinet.lowerId) ? '#b91c1c' : '#64748b'" text-anchor="middle" alignment-baseline="middle">{{ (info(cabinet.lowerId)?.name ?? '').slice(0, 25) }}</text>
          </g>
        </g>
      </svg>
    </div>
