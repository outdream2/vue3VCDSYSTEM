    ...
    <div class="relative min-h-0 flex-1 overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
      <svg :viewBox="`0 0 ${width} ${height}`" class="h-full w-full" preserveAspectRatio="xMidYMin meet">
        ...
        <g v-for="cabinet in cabinets" :key="cabinet.id">
          ...
          <rect :x="mapX2D(cabinet.position[2]) - panelWidth / 2" :y="mapY2D(cabinet.position[0]) - panelHeight / 2" :width="panelWidth" :height="panelHeight" :fill="selected.has(cabinet.upperId) || selected.has(cabinet.lowerId) ? 'url(#floor-selected)' : 'url(#floor-panel)'" :stroke="selected.has(cabinet.upperId) || selected.has(cabinet.lowerId) ? '#dc2626' : '#cbd5e1'" :stroke-width="selected.has(cabinet.upperId) || selected.has(cabinet.lowerId) ? '4' : '3'" rx="6" />
          ...
          <text :x="mapX2D(cabinet.position[2]) - panelWidth / 2 + 34" :y="mapY2D(cabinet.position[0]) - panelHeight / 2 + 180" font-size="68" font-weight="700" :fill="selected.has(cabinet.upperId) ? '#b91c1c' : '#64748b'" text-anchor="start" alignment-baseline="middle">{{ (info(cabinet.upperId)?.name ?? '').slice(0, 25) }}</text>
          <g v-if="cabinet.upperId !== cabinet.lowerId">
            ...
            <text :x="mapX2D(cabinet.position[2]) - panelWidth / 2 + 34" :y="mapY2D(cabinet.position[0]) - panelHeight / 2 + 364" font-size="68" font-weight="700" :fill="selected.has(cabinet.lowerId) ? '#b91c1c' : '#64748b'" text-anchor="start" alignment-baseline="middle">{{ (info(cabinet.lowerId)?.name ?? '').slice(0, 25) }}</text>
          </g>
        </g>
      </svg>
    </div>
